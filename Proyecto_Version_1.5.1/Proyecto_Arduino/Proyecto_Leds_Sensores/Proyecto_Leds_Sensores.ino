...
boolean Led7Encedido=false;
boolean Primera_Deteccion=true;
boolean Primera_Deteccion_Patio=true;
int ValorSensor;
...
void loop() 
{
  ...
  if(digitalRead(Sensor1)==HIGH && Primera_Deteccion==true)
    ...
    //CONTROL DEL SENSOR DEL PATIO//

     if(digitalRead(Sensor2)==LOW && Primera_Deteccion_Patio==true)
    {       
          Serial.print("SE2\n");
          Buffer = Buffer + "SE2-";
          Primera_Deteccion_Patio=false;
          Establecer_Tiempo();
          delay(150);
     }else if(digitalRead(Sensor2) == HIGH && Primera_Deteccion_Patio==false)
       {
          Serial.print("SA2\n");
          Establecer_Tiempo();
          Buffer = Buffer + "SA2-";
          Primera_Deteccion_Patio=true;
          delay(150);
       }//FIN CONTROL SENSOR DEL PATIO//
  ...
  if(Serial.available() > 0)
  {
      ...
      //ME ENCENDIERON EL LED7?//
      if(MensajeRecibido=="LE7")
      { 
        digitalWrite(LED7, HIGH);
        Buffer = Buffer+"LE7-";
        if(Led7Encedido=true)
        {
               Led7Encedido=!Led7Encedido;
        }
        //REGISTRO DE TIMEPO DEL LED//
          Establecer_Tiempo();
      }//FIN ENCENDIERON LED7//

      //ME APAGARON EL LED7?//
      if(MensajeRecibido=="LA7")
      {
        digitalWrite(LED7, LOW);
        Buffer = Buffer+"LA7-";
        if(Led7Encedido=false)
        {
               Led7Encedido=!Led7Encedido;
        }
