...
PFont fuente;  //variable para elegir la fuente de escritura//
...
boolean CambioDeColor1=true; //variables que controlan el estado de las luces de los leds en el plano//
...
int EncenderLed=1; //variables que controlan que hacer con los leds en las funciones//
int ApagarLed=2;

//VARIABLE PARA LA IMAGEN DE LA PLANTA Y DEL MENU//

PImage fondo;  //variables para tratar las 2 imagenes que manejamos en la interfaz//
PImage fondoMenu;


//VARIABLES PARA EL MANEJO DEL MENU//
int Selector_Menu = 0; // El 0 representa que estoy en el menu principal, 1 voy al boton IR que me lleva al plano//
...
//------------------------------------CLASE DISPOSITIVOS (para trabajar con cada dispositivo físico como un objeto)-------------------------------------------------------------------------//
...
void setup() 
{
  ...
  fondo = loadImage("Plano.jpg");  //cargo las 2 imagenes//
  fondoMenu = loadImage("FondoMenu.jpg");
  
  fuente = createFont("Comic Sans MS", 24); //configuro que tipo de fuente de letra voy a usar//
  textFont(fuente);
...
void draw() 
{
  ...
  if(Selector_Menu == 0) //Si estoy en el menu principal//
  {
    background(100);
    ...
    if(enRect(30,60,100,30)==true && mousePressed) // si me hacen click en GUARDAR guardo los datos del buffer//
    {
        ...
        textSize(20);
        fill(0);
        text("Se guardó correctamente...",70+200,70);
        
    }
    
    
  } //Fin si estoy en el menu principal//
