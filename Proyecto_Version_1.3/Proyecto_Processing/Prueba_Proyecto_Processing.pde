...
String MensajeReplay;
Serial myPort;
Serial myPort2;
String [] DividirReplayAcciones;
String [] DividirReplayTiempo;
String [] DividirReplayGeneral;
 String MensajeReciboBuffer;
...
boolean Pido_Primera_Vez_Fondo_Menu = true;
...
void setup() 
{
  ...
  if(Puertos.length>1)
  { 
  //  myPort2 = new Serial(this,Puertos[1], 9600);
  }
...
void draw() 
{
 ...
 if(Selector_Menu == 1) //si me seleccionaron IR//
 {
   ...
   {   
      ...
      } // if de si mensaje lectura es distinto de null 
     

}  //Barra de cierre de aviable
   ...
   if(Selector_Menu == 2)
       {
            if(Pido_Primera_Vez == true)
            {
              ...
              myPort.write(MensajeBuffer);
              delay(300);
              MensajeReciboBuffer=myPort.readStringUntil('\n');
              DividirReplayGeneral = split (MensajeReciboBuffer, '?'); 
              
              if(MensajeReciboBuffer!=null)
              {
              DividirReplayAcciones=split(DividirReplayGeneral[0],'-');
              DividirReplayTiempo=split(DividirReplayGeneral[1],'-');
              }
              Pido_Primera_Vez = false;
            ...
            if(Contador <= DividirReplayAcciones.length-1)
            {
             ...
             // DIBUJO LED 1 //
             print("delay: ");
             print(DividirReplayTiempo[Contador]);
             
              Delay = int (DividirReplayTiempo[Contador]);
            ...
            } // FIN IF CONDICION NULL DE BUFFER // 
